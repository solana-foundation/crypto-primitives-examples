...
Three tabs, all generating proofs client-side (WebAssembly, `@solana/zk-sdk`) and submitting them to the native verifier:
...
- **Confidential transfer** — Alice sends Bob encrypted amounts; balances update by homomorphic add/subtract and stay encrypted, while a range proof enforces that Alice can't overspend — without the chain ever seeing a balance.
...
just integration-test   # Mollusk integration tests (altbn128_g2, bls12_381)
```
...
| `/elgamal`  | ZK ElGamal → encrypted-value proofs (3 tabs)      |
