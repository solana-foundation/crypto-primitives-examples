use {
    mollusk_svm::{result::Check, Mollusk},
    solana_instruction::Instruction,
    solana_pubkey::Pubkey,
    std::str::FromStr,
};

const PROGRAM_ID: &str = "EgJAPMy5V2j442dTGFRqT5ZtPCWtg6BEbEo2QzkExYyw";

const IX_G1_ADD: u8 = 3;
const IX_G1_SUB: u8 = 4;
const IX_G1_MUL: u8 = 5;
const IX_G2_ADD: u8 = 6;
const IX_G2_SUB: u8 = 7;
const IX_G2_MUL: u8 = 8;

const SCALAR_SIZE: usize = 32;

// Big-endian BLS12-381 points: GENERATOR, 2*GENERATOR, 3*GENERATOR, generated
// from blstrs (the library agave's syscall uses). G1 = 96 bytes, G2 = 192 bytes.
const G1_GEN: &str = "17f1d3a73197d7942695638c4fa9ac0fc3688c4f9774b905a14e3a3f171bac586c55e83ff97a1aeffb3af00adb22c6bb08b3f481e3aaa0f1a09e30ed741d8ae4fcf5e095d5d00af600db18cb2c04b3edd03cc744a2888ae40caa232946c5e7e1";
const G1_TWO: &str = "0572cbea904d67468808c8eb50a9450c9721db309128012543902d0ac358a62ae28f75bb8f1c7c42c39a8c5529bf0f4e166a9d8cabc673a322fda673779d8e3822ba3ecb8670e461f73bb9021d5fd76a4c56d9d4cd16bd1bba86881979749d28";
const G1_THREE: &str = "09ece308f9d1f0131765212deca99697b112d61f9be9a5f1f3780a51335b3ff981747a0b2ca2179b96d2c0c9024e5224032b80d3a6f5b09f8a84623389c5f80ca69a0cddabc3097f9d9c27310fd43be6e745256c634af45ca3473b0590ae30d1";

const G2_GEN: &str = "";
const G2_TWO: &str = "";
const G2_THREE: &str = "";

fn mollusk() -> (Mollusk, Pubkey) {
    std::env::set_var(
        "SBF_OUT_DIR",
        concat!(env!("CARGO_MANIFEST_DIR"), "/../target/deploy"),
    );
    let program_id = Pubkey::from_str(PROGRAM_ID).unwrap();
    (Mollusk::new(&program_id, "crypto_primitives"), program_id)
}

fn from_hex(s: &str) -> Vec<u8> {
    (0..s.len())
        .step_by(2)
        .map(|i| u8::from_str_radix(&s[i..i + 2], 16).unwrap())
        .collect()
}

fn run(discriminator: u8, left: &[u8], right: &[u8], expected: &[u8], label: &str) {
    let (mollusk, program_id) = mollusk();

    let mut data = vec![discriminator];
    data.extend_from_slice(left);
    data.extend_from_slice(right);
    let instruction = Instruction::new_with_bytes(program_id, &data, vec![]);

    let result = mollusk.process_and_validate_instruction(
        &instruction,
        &[],
        &[Check::success(), Check::return_data(expected)],
    );

    println!("{} compute units: {}", label, result.compute_units_consumed);
}

fn scalar_three() -> Vec<u8> {
    let mut s = vec![0u8; SCALAR_SIZE];
    s[SCALAR_SIZE - 1] = 3;
    s
}

#[test]
fn g1_add_generator_plus_double_is_triple() {
    run(
        IX_G1_ADD,
        &from_hex(G1_GEN),
        &from_hex(G1_TWO),
        &from_hex(G1_THREE),
        "BLS12-381 G1 add (G + 2G)",
    );
}

#[test]
fn g1_sub_triple_minus_double_is_generator() {
    run(
        IX_G1_SUB,
        &from_hex(G1_THREE),
        &from_hex(G1_TWO),
        &from_hex(G1_GEN),
        "BLS12-381 G1 sub (3G - 2G)",
    );
}

#[test]
fn g1_mul_generator_times_three_is_triple() {
    run(
        IX_G1_MUL,
        &scalar_three(),
        &from_hex(G1_GEN),
        &from_hex(G1_THREE),
        "BLS12-381 G1 mul (G * 3)",
    );
}

#[test]
fn g2_add_generator_plus_double_is_triple() {
    run(
        IX_G2_ADD,
        &from_hex(G2_GEN),
        &from_hex(G2_TWO),
        &from_hex(G2_THREE),
        "BLS12-381 G2 add (G + 2G)",
    );
}

#[test]
fn g2_sub_triple_minus_double_is_generator() {
    run(
        IX_G2_SUB,
        &from_hex(G2_THREE),
        &from_hex(G2_TWO),
        &from_hex(G2_GEN),
        "BLS12-381 G2 sub (3G - 2G)",
    );
}

#[test]
fn g2_mul_generator_times_three_is_triple() {
    run(
        IX_G2_MUL,
        &scalar_three(),
        &from_hex(G2_GEN),
        &from_hex(G2_THREE),
        "BLS12-381 G2 mul (G * 3)",
    );
}
