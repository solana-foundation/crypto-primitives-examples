...
Program upgrades to devnet run through the `Release` GitHub Actions workflow. The workflow builds a verified program, writes an upgrade buffer, upgrades the program, and uploads the IDL — all using a deployer keypair loaded from Doppler.
...
Trigger the `Release` workflow from the Actions tab (`workflow_dispatch`), optionally setting the priority fee. It loads `RPC_URL` and `DEPLOYER_KEYPAIR` from Doppler, builds verified, writes a buffer, upgrades, and uploads the IDL.
...
solana-verify verify-from-repo \
  --program-id EgJAPMy5V2j442dTGFRqT5ZtPCWtg6BEbEo2QzkExYyw \
  --url https://github.com/solana-foundation/crypto-primitives-examples
```
