...
function memberData(point: any, disc: number) {
    return new Uint8Array([disc, ...point.toBytes(false)]);
}

async function readRegistry(account: any) {
...
async function main() {
    ...
    const sign = (subset: any[]) => bls.aggregateSignatures(subset.map(m => bls.sign(hashed, m.secretKey)));
