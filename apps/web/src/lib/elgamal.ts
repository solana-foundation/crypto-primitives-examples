...
/** Adds two ciphertexts under the same key; the result encrypts the sum of the amounts. */
export function addAmountToCiphertext(balance: Uint8Array, amount: Uint8Array): Uint8Array {
    return sumCiphertexts([balance, amount]);
}

/** Subtracts one ciphertext from another under the same key; the result encrypts the difference. */
export function subtractAmountFromCiphertext(balance: Uint8Array, amount: Uint8Array): Uint8Array {
    const commitment = Point.fromBytes(balance.subarray(0, 32)).subtract(Point.fromBytes(amount.subarray(0, 32)));
    const handle = Point.fromBytes(balance.subarray(32, 64)).subtract(Point.fromBytes(amount.subarray(32, 64)));
    const out = new Uint8Array(64);
    out.set(commitment.toBytes(), 0);
    out.set(handle.toBytes(), 32);
    return out;
