...
export function BlsRegistryDemo() {
    ...
    async function ensureRegistry(payer: TransactionSigner): Promise<Address> {
    ...
    async function addMember() {
        ...
        try {
            ...
            const registryAddress = await ensureRegistry(signer);
    ...
    async function toggleIn(row: Row) {
        ...
        try {
            ...
            const signature = await signAndSend([instructionFor(member, registry, discriminator)], signer);
        ...
        <div className="space-y-5 rounded-xl border bg-card p-5">
            <div>
                ...
                <ol className="mt-2 list-decimal space-y-1 pl-5 text-sm text-muted-foreground">
                    <li>
                        Add members — each key is folded into one aggregate key on-chain with G2 addition; the program
                        stores that single key, not the list.
                    </li>
                    <li>Remove a member — G2 subtraction takes their key back out, the op alt_bn128 doesn't have.</li>
                    <li>
            ...
            <div className="flex flex-wrap items-center gap-3">
                <Button disabled={!ready} loading={busy === 'add'} onClick={() => void addMember()}>
                    {!isLocalnet && !ready ? 'Connect wallet to add' : 'Add member'}
                </Button>
                <Button
            ...
            {registry && (
                <div className="space-y-1 text-xs font-medium text-sand-1100">
                    ...
                    <div>click a member to toggle whether they sign · × removes on-chain</div>
                </div>
            )}
