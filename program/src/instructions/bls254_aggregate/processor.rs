use pinocchio::{account::AccountView, Address, ProgramResult};

use crate::{
    errors::CryptoPrimitivesProgramError,
    syscall::{alt_bn128_group_op, set_return_data},
};

const ALT_BN128_PAIRING: u64 = 3;
const ALT_BN128_G2_ADD: u64 = 4;

const G1_POINT: usize = 64;
const G2_POINT: usize = 128;
const HEADER: usize = G1_POINT * 2;
const PAIRING_INPUT: usize = (G1_POINT + G2_POINT) * 2;

/// Canonical BN254 G2 generator, big-endian (EIP-197 encoding).
const G2_GENERATOR: [u8; G2_POINT] = [
    ,
];

/// Processes the Bls254AggregateVerify instruction.
///
/// # Layout
/// `aggregate_signature` (G1, 64 bytes) ‖ `negated_message_hash` (G1, 64 bytes)
/// ‖ one or more big-endian G2 public keys (128 bytes each).
///
/// Aggregates the public keys on-chain via the G2 addition syscall, then checks
/// the BLS pairing relation `e(aggSig, G2) · e(-H(m), aggPk) == 1` using the
/// pairing syscall. Returns the on-chain aggregate public key as return data.
pub fn process_bls254_aggregate_verify(
    _program_id: &Address,
    _accounts: &[AccountView],
    instruction_data: &[u8],
) -> ProgramResult {
    if instruction_data.len() < HEADER + G2_POINT
        || !(instruction_data.len() - HEADER).is_multiple_of(G2_POINT)
    {
        return Err(CryptoPrimitivesProgramError::InvalidInputLength.into());
    }

    let aggregate_signature = &instruction_data[..G1_POINT];
    let negated_message_hash = &instruction_data[G1_POINT..HEADER];
    let pubkeys = &instruction_data[HEADER..];
    let pubkey_count = pubkeys.len() / G2_POINT;

    let mut aggregate_pubkey = [0u8; G2_POINT];
    aggregate_pubkey.copy_from_slice(&pubkeys[..G2_POINT]);

    let mut add_input = [0u8; G2_POINT * 2];
    for i in 1..pubkey_count {
        add_input[..G2_POINT].copy_from_slice(&aggregate_pubkey);
        add_input[G2_POINT..].copy_from_slice(&pubkeys[i * G2_POINT..(i + 1) * G2_POINT]);
        alt_bn128_group_op(ALT_BN128_G2_ADD, &add_input, &mut aggregate_pubkey)?;
    }

    let mut pairing_input = [0u8; PAIRING_INPUT];
    pairing_input[..G1_POINT].copy_from_slice(aggregate_signature);
    pairing_input[G1_POINT..G1_POINT + G2_POINT].copy_from_slice(&G2_GENERATOR);
    pairing_input[G1_POINT + G2_POINT..HEADER + G2_POINT].copy_from_slice(negated_message_hash);
    pairing_input[HEADER + G2_POINT..].copy_from_slice(&aggregate_pubkey);

    let mut result = [0u8; 32];
    alt_bn128_group_op(ALT_BN128_PAIRING, &pairing_input, &mut result)?;

    let valid = result[31] == 1 && result[..31].iter().all(|&byte| byte == 0);
    if !valid {
        return Err(CryptoPrimitivesProgramError::AggregateVerifyFailed.into());
    }

    set_return_data(&aggregate_pubkey);
    Ok(())
}
